    ...
    */
    $scope.getEnvironmentalData = function(startTime, endTime, size) {
      ...
      CurrentServices.getEnvOccupancy($scope.uaaToken, startTime, endTime, size).then(function(data){
        $scope.evnData.occupancy = '';
        var prop = '';
        var unit = '';
        if(data && data.content) {
          if(data.content.length > 0) {
            var content = data.content[0];
            if(content && content.measures) {
              prop = content.measures;
            }
            if(content && content.properties) {
              unit = content.properties.Unit;
            }
          }
          $scope.evnData.occupancy = prop.Value === 0 ?   'Unoccupied' : 'Occupied';
        }
      });
      ...
      CurrentServices.getEnvTemp($scope.uaaToken, startTime, endTime, size).then(function(data){
        ...
        if(data && data.content) {
          ...
          $scope.evnData.temp = prop.Value + '° ' + unit;
        }
      });
      ...
      CurrentServices.getEnvLight($scope.uaaToken, startTime, endTime, size).then(function(data){
        ...
        if(data && data.content) {
          ...
          $scope.evnData.light = prop.Value + ' ' + unit;
        }
      });
    };
    ...
    */
    $scope.getIndoorPositioningData = function(startTime, endTime) {
      ...
      CurrentServices.getHistoricalPosition($scope.uaaToken, startTime, endTime).then(function(data){
        var prop = {'x':40,'y':454,'z':0,'orientation':6,'durationInSec':1};
        if (! $scope.indoorData) {
          $scope.indoorData = {};
        }
        ...
        if(data && data.content) {
          ...
          $scope.indoorData.positionData = '(' +  prop.x + ', '+ prop.y + ', '+ prop.z + ');  Duration: ' + prop.durationInSec + ' Seconds; Orientation: ' + prop.orientation;
        }
      });
